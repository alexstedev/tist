...
def cli():
    ...
    rcfile = os.path.expanduser(TISTRC)
    ...
    if os.path.exists(rcfile):
        ...
        if (
            "todoist" not in config
            or "api_key" not in config["todoist"]
            and not is_help_cmd
        ):
            log.error("Run configure first. Exiting.")
            exit(1)

        todoist = TodoistAPI(config["todoist"]["api_key"])
...
def configure(map_project, map_tag, todoist_api_key):
    ...
    data["taskwarrior"]["project_sync"] = {k: True for k in map_project.values()}
...
def sync():
    ...
    log.important("Starting to sync tasks from Taskwarrior...")
    for tw_task in tw_tasks["pending"]:
        ...
        if tw_task["project"] not in config_ps or not config_ps[tw_task["project"]]:
        ...
        if "todoist_id" in tw_task:
            ti_task = todoist.get_task(tw_task["todoist_id"])
            if ti_task is None:
                # Ti task has been deleted
                taskwarrior.task_delete(uuid=tw_task["uuid"])
                continue

            ti_task = ti_task.__dict__
    ...
    for ti_task in tasks:
        ...
        if c_ti_task["project"] not in config_ps or not config_ps[c_ti_task["project"]]:
            log.warn(f"Ignoring Task {desc} ({project})")
            continue
...
def _convert_ti_task(ti_task, ti_project_list):
    ...
    # Tags
    # data["tags"] = [
    #     utils.try_map(config["todoist"]["tag_map"], todoist.get_label(l_id)["name"])
    #     for l_id in ti_task["labels"]
    # ]

    # Dates
    data["entry"] = utils.parse_date(ti_task["created_at"])
    data["due"] = utils.parse_due(utils.try_get_model_prop(ti_task, "due"))

    data["status"] = "completed" if ti_task["is_completed"] else "pending"
...
def _ti_update_task(tw_task, ti_project_list):
    ...
    with log.on_error(f"Todoist update '{description}' ({project})"):
        ...
        if tw_task["description"] != ti_task["content"]:
            ti_task["content"] = tw_task["description"]
            changed = True
        ...
        if ti_task["priority"] != priority and priority != 0:
            ti_task["priority"] = priority
            changed = True

        if (not ti_task["is_completed"] and tw_task["status"] == "completed") or (
            ti_task["is_completed"] and tw_task["status"] == "pending"
        ):
            changed = True
        ...
        if changed:
            ...
            todoist.update_task(tid, **ti_task)

            # Move to another project
            # TODO: Find alternative
            if ti_task["project_id"] != project["id"]:
                raise NotImplementedError(
                    "Moving tasks between projects is not supported"
                )

            # Open/close ti task
            if ti_task["is_completed"] and tw_task["status"] == "pending":
                todoist.reopen_task(tid)
            elif not ti_task["is_completed"] and tw_task["status"] == "completed":
...
def _ti_add_task(tw_task, ti_project_list):
    ...
    with log.on_error(f"Todoist add '{description}' ({project})"):
        ...
        ti_task = todoist.add_task(tw_task["description"], **data)
...
def _ti_project_list():
    result = {}
    for p in todoist.get_projects():
        p = p.__dict__
        project_hierarchy = [p]
        pp = p
        while pp["parent_id"]:
            pp = todoist.get_project(p["parent_id"])
            project_hierarchy.insert(0, pp)

        project_name = ".".join(p["name"] for p in project_hierarchy)
        project_name = utils.try_map(config["todoist"]["project_map"], project_name)
