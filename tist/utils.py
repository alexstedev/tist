...
def parse_recur_string(date_string):
    ...
    result = (
        _recur_single_cycle(date_string)
        or _recur_multi_cycle(date_string)
        or _recur_day_of_week(date_string)
        or _recur_day_of_month(date_string)
        or _recur_special(date_string)
    )
    if not result:
...
RE_SINGLE_CYCLE = re.compile(rf"^(({_EVERY}\s(1\s)?{_PERIOD})|{_SIMPLE}){_IGNORED}$")

# A multi cycle recurrence is of the form: every N <period>s
RE_MULTI_CYCLE = re.compile(rf"^{_EVERY}\s({_CYCLES}|other)\s{_PERIOD}{_IGNORED}$")


# A day of week recurrence is of the form:
# - every (monday | tuesday | ...)
# - every Nth (monday | tuesday | ...)
RE_EVERY_DOW = re.compile(rf"^{_EVERY}\s(({_CYCLES}|{_OTHER})\s)?{_DOW}{_IGNORED}$")


# A day of month recurrence is of the form: every Nth
RE_EVERY_DOM = re.compile(rf"^{_EVERY}\s{_CYCLES}{_IGNORED}$")


# Other patterns that don't fit in with the others
RE_SPECIAL = re.compile(
    rf"^{_EVERY}\s(?P<label>morning|evening|weekday|workday|last\sday)$"
)
